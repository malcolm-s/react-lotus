...
export class Sheet extends Component {
  ...
  componentWillMount() {
    ...
    const keyMovements = normalKeyDowns
      ...
      .filter(reference =>
        isWithinBounds(
          reference,
          this.props.adapter.getXMax(),
          this.props.adapter.getYMax()
        ));

    const ctrlKeyMovements = ctrlKeyDowns.map(
      this._handleCtrlKeyMovement.bind(this)
    );

    const movementSubscription = Rx.Observable
      .merge(keyMovements, ctrlKeyMovements)
      .subscribe(reference =>
        this.setState({ selectedCellReference: reference }));
    ...
    const enterKeySubscription = keydowns
      ...
      .subscribe(() =>
        this.setState(current => ({
          enteredCellReference: current.selectedCellReference
        })));
  ...
  render() {
    const columns = this.props.adapter.getColumns(
      this.state.selectedCellReference,
      this.state.enteredCellReference
    );
    ...
    const selectedCell = this.props.adapter.getCell(
      this.state.selectedCellReference
    );
